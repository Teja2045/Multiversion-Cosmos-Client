...
type GovernanceInterface interface {
	...
	GetProposalV1Info(proposalID uint64) (*gov1.QueryProposalResponse, error)
...
func GrpcDial(address string, enc EncodingConfig) (*grpc.ClientConn, error) {
	return grpc.Dial(address, grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithDefaultCallOptions(
			grpc.ForceCodec(codec.NewProtoCodec(enc.InterfaceRegistry).GRPCCodec()),
		))
}
...
func (c *queryClient) GetProposalV1Info(proposalID uint64) (*gov1.QueryProposalResponse,
	error) {
	c.count++
	resp, err := c.gov.govCl1.Proposal(context.Background(),
		&gov1.QueryProposalRequest{ProposalId: proposalID})
	if err != nil {
		return nil, err
	}
...
func RegisterCodecForGov(encConfig *EncodingConfig) {
	...
	gov1.RegisterLegacyAminoCodec(encConfig.Amino)
	gov1.RegisterInterfaces(encConfig.InterfaceRegistry)
}
